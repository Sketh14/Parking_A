        ...
        private void WaitForVehicleToPass()
        {
            ...
            for (int i = 0; i < _vehicleInfos.Length; i++)
            {
                ...
                for (int j = 1; j > -2; j--)
                {
                    ...
                    tempRayPos.z += (_cGridCellSize + 0.1f) * Mathf.Abs(_vehicleInfos[i].InteractedDir.x) * (_vehicleInfos[i].VehicleType + 1) * j;     //+ (0.18f * _vehicleInfos[i].VehicleType);
